...
<body data-section="0" data-barba="wrapper">
  <div data-barba="container" data-barba-namespace="common"><?php get_header(); ?>
    <div>
      ...
      <article class="p-event-block">
        <div class="p-event-block__wrapper p-event-block__wrapper--another">
          <h4 class="p-event-block__block-title">SYMPOSIUM</h4><?php
          $title = get_field('symposium_title');
          if($title):
          ?>
          <h2 class="p-event-block__event-title"><?= $title; ?></h2><?php endif; ?>
          <?php
          $img_array = get_image_array('symposium_image');
          if($img_array != []):
          ?>
          <div class="p-event-block__media">
          ...
          if($text):
          ?>
          <p class="p-event-block__description p-main-text"><?= $text; ?></p><?php endif; ?>
          <?php
          $link = get_field('peatix_link');
          $text = get_field('peatix_link_text');
          if($link && $text):
          ?><a class="p-event-block__link-another js-hover" href="<?= $link; ?>" target="_blank" rel="noopener noreferrer"><span><?= $text; ?></span></a><?php endif; ?>
        </div>
      </article>
    </div><?php get_footer(); ?>
    <?php wp_footer(); ?>
