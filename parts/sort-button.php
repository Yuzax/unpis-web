...
<div class="c-sort">
  <div class="c-sort__wrap">
    <ul class="c-sort__category">
      <?php
      ...
      ?>
      <li><a class="js-change-color-target js-hover <?php if($current_cat->name == $category->name){ echo 'is-active'; }; ?><?php if($is_dark_mode){ echo ' is-rev'; }; ?>" href="<?= get_category_link($category->term_id); ?>" aria-label="<?= $category->name . 'でフィルター'; ?>"><?= $category->name; ?></a></li><?php endforeach; ?>
    </ul>
    <div class="c-sort__layout js-change-color-target <?php if($is_dark_mode){ echo ' is-rev'; }; ?>">
      <?php
      ...
      <button class="js-hover js-change-style__button js-change-style__button--list" type="button" aria-label="リストレイアウトにレイアウトを変更" aria-pressed="<?php echo $is_list_layout ? 'true' : 'false'; ?>">list</button>
    </div>
  </div>
</div>